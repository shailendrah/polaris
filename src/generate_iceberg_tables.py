"""
Generate fake data with Faker and write it as Apache Iceberg tables to OCI
Object Storage via its S3-compatible API.
...
Polaris's Oracle ADB metastore. Set USE_POLARIS=false to fall back to a local
SQLite catalog. Data + manifests + snapshot files live in OCI Object Storage.
...
# OCI Object Storage S3-compat target. Customer Secret Keys (Console →
# Profile → Customer Secret Keys) are sent as access-key/secret to the
# S3-compat endpoint at <namespace>.compat.objectstorage.<region>.oraclecloud.com.
...
# PyIceberg's underlying AWS SDK falls back to AWS_REGION/AWS_DEFAULT_REGION
# for SigV4 signing when not explicitly told the bucket region. If the shell
# still has AWS_REGION pointing at an old AWS region (e.g. us-west-1), every
# request to OCI will be signed for the wrong region and OCI rejects them
# with what looks like a 404 NoSuchBucket. Force the env to OCI's region.
os.environ["AWS_REGION"] = OCI_REGION
os.environ["AWS_DEFAULT_REGION"] = OCI_REGION
# OCI's S3-compat API doesn't accept Content-Encoding: aws-chunked on
# PutObject. AWS SDK v2 defaults to WHEN_SUPPORTED — switch both to
# WHEN_REQUIRED so chunked encoding is only added when truly needed.
os.environ["AWS_REQUEST_CHECKSUM_CALCULATION"] = "WHEN_REQUIRED"
os.environ["AWS_RESPONSE_CHECKSUM_VALIDATION"] = "WHEN_REQUIRED"

S3_PREFIX = os.environ.get("S3_PREFIX", "polaris-iceberg")
WAREHOUSE = f"s3://{OCI_BUCKET}/{S3_PREFIX}"
# OCI's vhcompat endpoint (Feb 2026 release) carries a wildcard cert covering
# <bucket>.vhcompat... so virtual-hosted-style addressing works without TLS
# errors. Buckets must be created via the S3 API (see create_oci_bucket.sh)
# to be eligible.
...
def make_catalog():
    ...
    # PyIceberg's S3 FileIO config — same shape for both REST and SQLite
    # catalogs. `s3.endpoint` redirects every S3 call at OCI's vhcompat host
    # which has a wildcard cert covering bucket subdomains, so default
    # virtual-hosted-style addressing works without TLS errors.
    s3_io = {
        "s3.region": OCI_REGION,
        "s3.access-key-id": OCI_ACCESS_KEY,
        "s3.secret-access-key": OCI_SECRET_ACCESS_KEY,
    ...
    if USE_POLARIS:
        return load_catalog(
            ...
            **{
                ...
                # own object-store creds, so we don't want to ask Polaris to
...
def write_table(catalog: SqlCatalog, name: str, arrow_table: pa.Table) -> None:
...
def main() -> None:
    ...
    print(
        ...
        f"--bucket-name {OCI_BUCKET} --prefix {S3_PREFIX}/"
    )
