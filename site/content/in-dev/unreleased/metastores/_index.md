...
Polaris supports two metastore implementations and persistence backends:
...
| Oracle (JDBC)| `relational-jdbc` | Ready to use  | Data is stored in an Oracle database using JDBC. Recommended for production.                    |

{{< alert warning >}}
The default `in-memory` backend is **not suitable for production**. Data will be lost when the server restarts!
{{< /alert >}}

This section explains how to configure and use Polaris with the following backend:
