# Polaris on ADW + AWS S3 Iceberg — End-to-End Demo
...
2. **Oracle Autonomous Data Warehouse (ADW)** stores Polaris's catalog
   metadata *and* doubles as the SQL query engine via `DBMS_CLOUD` external
   tables.
...
```
   PyIceberg ──REST commit──▶  Polaris  ──JDBC──▶  ADW (POLARIS_SCHEMA)
       │                                              │
       └──parquet+manifests──▶  S3  ◀──HTTPS──── ADW (LAKEHOUSE, DBMS_CLOUD)
```
...
```bash
# AWS — used by both PyIceberg (writer) and ADW external tables
export AWS_ACCESS_KEY_ID=...
export AWS_SECRET_ACCESS_KEY=...
export AWS_REGION=us-west-1

# ADW — wallet path, admin password, tnsnames alias
...
## 1. Provision `POLARIS_SCHEMA` on ADW

This creates the user that Polaris will use as its catalog metastore.

```
make polaris-down       # if already running — must stop before drop
make adw-reset
```
...
`INTERNAL`/`S3` rooted at `s3://${S3_BUCKET}/polaris-iceberg`.
...
write produces a new `metadata.json`, and ADW pins to a specific URL at table
...
- **A.** Connects as `ADMIN` to ADW, creates user `LAKEHOUSE` (or resets its
...
- **B.** Reconnects as `LAKEHOUSE`, creates `AWS_S3_CRED` from the AWS keys.
  *(ADW has to fetch S3 objects directly — Polaris doesn't proxy data.)*
...
3. ADW external tables still point at the **old** metadata.json (DBMS_CLOUD
...
make adw-reset                    # wipe POLARIS_SCHEMA on ADW

# In ADW as ADMIN, optional:
...
| `ORA-28219 Password contains the username` on `CREATE USER lakehouse IDENTIFIED BY 'Lakehouse#...'` | ADW's mandatory password profile forbids the username (case-insensitive) anywhere in the password. Use a value without `lakehouse` in it. |
| `ORA-28007 The password cannot be reused` on `ALTER USER lakehouse IDENTIFIED BY ...` | ADW enforces password history. The script avoids hitting this on re-runs by ALTERing only when `rotate_lakehouse_pwd='Y'`; to actually rotate, also bump `&lakehouse_pwd` to a brand-new value. |
| ADW external table returns 0 rows after a fresh PyIceberg run | Externals are pinned to the old metadata.json — re-run sections 6–7. |
