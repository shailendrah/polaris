...
Polaris supports Oracle (JDBC) as the production metastore. See the [Persistence]({{% relref "persistence" %}}) page for detailed configuration instructions.
...
Example for the Oracle metastore:
...
The command above bootstraps a realm named `polaris-realm1` with root password: `$ROOT_PASSWORD`.
