...
tasks.withType(Test::class.java).configureEach {
  ...
  // Java 24+ removed Security Manager enablement; only set this for older runtimes.
    ...
    it.configure {
      ...
      jvmArgumentProviders.add(
        CommandLineArgumentProvider {
          ...
          // code).
          val securityManagerAllow =
            if (JavaVersion.current().majorVersion.toInt() >= 24) "" else "-Djava.security.manager=allow"
          ...
          var quarkusTestArgLine =
            if (explicitQuarkusTestArgLine != null)
              listOf(explicitQuarkusTestArgLine, securityManagerAllow).filter { it.isNotBlank() }.joinToString(" ")
