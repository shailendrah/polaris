...
-- s3://${S3_BUCKET}/polaris-iceberg/ as Oracle external tables. After this,
...
-- S3 host for the iceberg data. The DBMS_CLOUD iceberg engine uses a separate
-- code path from the regular CREATE_EXTERNAL_TABLE, and that path does NOT
-- auto-grant the network ACL via the credential — we have to APPEND_HOST_ACE
-- explicitly. Adjust if you change the bucket.
DEFINE s3_host = 'skmawsbucket1.s3.us-west-1.amazonaws.com'

-- LAKEHOUSE password. ADW's mandatory profile rules:
...
-- alone: ADW's mandatory profile rejects reusing a previous password
-- (ORA-28007), so an unconditional ALTER would fail on every re-run after
-- the first. To rotate the password, set rotate_lakehouse_pwd=Y AND bump
-- &lakehouse_pwd to a value never used before.
